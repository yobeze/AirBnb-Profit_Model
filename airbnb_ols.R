...
use_df$has_parking <- ifelse(grepl("parking", use_df$amenities, ignore.case = T), 1, 0)
...
plot(log_price~host_yrs_exp, data = use_df)
...
model_nbhd <- lmer(log_price ~ accommodates+bedrooms+beds+n_bathrooms+bathroom_private+
                 room_private+entire_prop+
                 has_ac+ has_fridge + has_wifi + has_heating + has_parking + has_tv+
                 host_yrs_exp+host_is_superhost.t+
                 number_of_reviews+review_scores_rating+
...
hist(resid(model_all))
{qqnorm(resid(model_all))
  qqline(resid(model_all))}
...
# AIC
# when I've run more model versions, I'll compare them with AIC
